...
if __name__ == '__main__':
    ...
    def _has_nifti(d):
        return glob(os.path.join(d, "*.nii.gz")) or glob(os.path.join(d, "*.nii"))

    assert os.path.isdir(args.input_dir), "Specified input directory does not exist: {}".format(args.input_dir)
    if _has_nifti(args.input_dir):
        data_dirs = [args.input_dir]
    else:
        subdirs = [y for y in list(set(os.path.dirname(x) for x in glob(os.path.join(args.input_dir, "*/*.nii*"))))]
        data_dirs = [d for d in subdirs if _has_nifti(d)]
        if not data_dirs:
