...
def _find_nifti_with_suffix(directory, suffix):
    """Return list of nifti paths matching suffix with .nii.gz or .nii"""
    gz = glob(os.path.join(directory, f"*{suffix}.nii.gz"))
    nii = glob(os.path.join(directory, f"*{suffix}.nii"))
...
def batch_mask(infer_direcs, param_file, out_dir, suffix, checkpoint='last', overwrite=False, thresh=0.5, clean=False,
    ...
    for n, direc in enumerate(infer_direcs):
        ...
        if skip:
            continue
        # determine mask output path (write to out_dir[n] if provided, else current directory)
...
if __name__ == '__main__':
    ...
    root_files = glob(os.path.join(args.input_dir, "*.nii.gz")) + glob(os.path.join(args.input_dir, "*.nii"))
    if root_files:
        data_dirs = [args.input_dir]
    else:
        nested = glob(os.path.join(args.input_dir, "*/*.nii.gz")) + glob(os.path.join(args.input_dir, "*/*.nii"))
        if nested:
            data_dirs = [y for y in list(set(os.path.dirname(x) for x in nested))]
        else:
            raise FileNotFoundError("No image data (.nii/.nii.gz) found in input directory!")
