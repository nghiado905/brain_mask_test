...
def _load_single_study(study_dir, file_prefixes, data_format, out_res=None, out_dims=None, res_order=2, plane=None,
    ...
    def _find_nifti(prefix):
        gz = glob(study_dir + '/*' + prefix + '.nii.gz')
        nii = glob(study_dir + '/*' + prefix + '.nii')
        return gz if gz else nii

    images = []
    for contrast in file_prefixes:
        paths = _find_nifti(contrast)
        if not paths:
            raise ValueError(f"No matching image files found for prefix: {contrast}")
