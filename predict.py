...
def _find_nifti_with_prefix(directory, prefix):
    """Find files matching *prefix(.nii|.nii.gz) in directory."""
    gz = glob(os.path.join(directory, f"*{prefix}.nii.gz"))
    nii = glob(os.path.join(directory, f"*{prefix}.nii"))
    return gz if gz else nii
...
def predictions_2_nii(predictions, infer_dir, out_dir, params, mask=None, thresh=None):
    ...
    src_nii = _find_nifti_with_prefix(infer_dir, params.data_prefix[0])
    if not src_nii:
        raise FileNotFoundError(f"No NIfTI found for prefix {params.data_prefix[0]} in {infer_dir}")
    nii1 = nib.load(src_nii[0])
    affine = nii1.affine
    name_prefix = os.path.basename(infer_dir[0:-1] if infer_dir.endswith('/') else infer_dir)

    # 3D inference (reconstruct on CPU to giảm tải GPU bộ nhớ)
    with tf.device('/CPU:0'):
        predictions = reconstruct_infer_patches_3d(predictions, infer_dir, params)

    # mask predictions based on provided mask
    if mask:
        mask_nii = _find_nifti_with_prefix(infer_dir, mask)
        if not mask_nii:
            raise FileNotFoundError(f"No mask found for prefix {mask} in {infer_dir}")
        mask_img = nib.load(mask_nii[0]).get_fdata() > 0
...
if __name__ == '__main__':
    ...
    if args.spec_direc:
        if os.path.isdir(args.spec_direc) and all([_find_nifti_with_prefix(args.spec_direc, item) for item in
    ...
    study_dirs = [study for study in study_dirs if all(
        [_find_nifti_with_prefix(study, item) for item in my_params.data_prefix])]
